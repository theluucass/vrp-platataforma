...
# ------------------ VRP ------------------
def solve_vrp(locations, num_vehicles, depots):
    distance_matrix = [[int(haversine(a, b)) for b in locations] for a in locations]
...
def solve_cvrp(locations, demands, capacities, depots):
    n = len(locations)
    matrix = [[int(haversine(locations[i], locations[j])) for j in range(n)] for i in range(n)]
...
def solve_vrptw(locations, time_windows, num_vehicles, depots):
    matrix = [[int(haversine(locations[i], locations[j]) / 8.33) for j in range(len(locations))] for i in range(len(locations))]
    starts = depots * (num_vehicles // len(depots)) + depots[:num_vehicles % len(depots)]
...
# ------------------ INTERFAZ ------------------
st.title("Optimización de Rutas (VRP / CVRP / VRPTW)")
tab = st.selectbox("Selecciona el tipo de problema", ["VRP", "CVRP", "VRPTW"])

with st.form("form"):
    n = st.number_input("Número de paradas", min_value=2, value=4)
    locations = []
    time_windows = []
    for i in range(n):
    ...
    if tab == "CVRP":
        demands = [st.number_input(f"Demanda en parada {i}", min_value=0, value=1) for i in range(n)]
        capacities = [st.number_input(f"Capacidad del vehículo {j}", min_value=1, value=10) for j in range(int(v))]

    submitted = st.form_submit_button("Calcular Ruta")

if submitted:
    if tab == "VRP":
        result = solve_vrp(locations, int(v), depots)
        if result:
            for i, route in enumerate(result):
                st.success(f"Vehículo {i + 1}: {' → '.join(map(str, route))}")
        else:
